...
export const Route = createFileRoute("/routine/$studentId/")({
  head: () => ({
    meta: [
    ...
    try {
      ...
      const exercisesData: GetDailyStudentExerciseDto[] =
        await getDailyStudentExercisesByStudentIdAndDates(
          ...
          console.warn("No se pudieron obtener ejercicios de la semana:", err);
          return [];
        });
      ...
      return { weekRoutineDays, studentId: params.studentId };
    } catch (error) {
    ...
    try {
      ...
      const exercisesData: GetDailyStudentExerciseDto[] =
        await getDailyStudentExercisesByStudentIdAndDates(
          ...
          console.warn("Error al obtener ejercicios:", err);
          return [];
        });
      ...
      const nextWeekDays: DayRoutine[] = Array.from({ length: 7 }, (_, i) => {
      ...
      setWeekRoutineDays(nextWeekDays);
    } catch (error) {
    ...
    <AppShell>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
        {weekRoutineDays.map((day) => {
          ...
          const dayParam = day.scheduledDate ? day.scheduledDate.split("T")[0] : "";
          return (
            <Link
              key={day.id}
              to="/routine/$studentId/$dayId"
              params={{ studentId: studentId!, dayId: dayParam }}
              className="group"
            >
              <Card
