...
import { ArrowLeft, Save, UserCog, Flame, Trophy, Shield, FileText } from "lucide-react";
...
export const Route = createFileRoute("/clients/$studentId_/edit")({
  head: () => ({
    meta: [
      { title: "Editar Alumno — PYROSFIT" },
      { name: "description", content: "Edición y ajuste de métricas del alumno." },
    ],
  }),
  beforeLoad: ({ location }) => {
  ...
  loader: async ({ params }) => {
    ...
    try {
      const [studentData, studentStreakData] = await Promise.all([
        getStudent(studentId).catch((err) => {
...
function ClientEditPanelPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      navigate({ to: "/clients/$studentId", params: { studentId: studentId.toString() } });
    } catch (error) {
        ...
        <Card className="bg-gradient-card border-border shadow-elevated p-5 sm:p-7">
          ...
          <div className="flex items-center justify-end gap-3">
            <Button
              ...
              onClick={() => navigate({ to: "/clients/$studentId", params: { studentId: studentId.toString() } })}
            >
              Cancelar
            </Button>
            <Button
